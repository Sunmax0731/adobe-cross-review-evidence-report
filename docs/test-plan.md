...
```powershell
cd D:\AI\AdobePlugin\Bridge\adobe-cross-review-evidence-report
npm test
npm run validate
npm run build
```
...
- `npm run validate` で `dist/adobe-cross-review-evidence-report-representative-report.json` を生成する。
