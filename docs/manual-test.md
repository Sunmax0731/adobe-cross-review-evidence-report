...
- 作業ディレクトリ: `D:\AI\AdobePlugin\Bridge\adobe-cross-review-evidence-report`
...
2. `npm run validate` を実行し、`dist/adobe-cross-review-evidence-report-representative-report.json` が生成されることを確認する。
3. レポート JSON を開き、happy-path、missing-required、warning、mixed-batch の summary が期待値通りであることを確認する。
4. Bridge 向け host adapter の targetHost と report summary を確認する。
