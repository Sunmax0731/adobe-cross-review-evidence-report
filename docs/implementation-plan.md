...
1. 要件定義、仕様、設計を NON PICKUP Rank 7 と ZIP metadata から再構成。
...
6. `dist/adobe-cross-review-evidence-report-docs.zip` を生成。

## 次フェーズ

- 実ユーザーの手動テスト結果を反映する。
- Bridge 向け host adapter とローカル証跡レポート CLI を実データで拡張する。
